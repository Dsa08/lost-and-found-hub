...
class _UserManagementScreenState extends ConsumerState<UserManagementScreen> {
  ...
  // ── Edit Poin ──────────────────────────────────────────────────────────────
  Future<void> _editPoin(
      BuildContext context, String userId, int currentPoin) async {
    final ctrl = TextEditingController(text: '$currentPoin');
    final result = await showDialog<int>(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text('Edit Total Poin',
            style: TextStyle(fontWeight: FontWeight.w700)),
        content: TextField(
          controller: ctrl,
          keyboardType: TextInputType.number,
          decoration: const InputDecoration(
              labelText: 'Jumlah Poin', suffixText: 'poin'),
        ),
        actions: [
          TextButton(
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: Text('✅ Poin diubah menjadi $result'),
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text((data['nama'] as String? ?? 'U')[0].toUpperCase(),
            ...
            _DetailRow('Total Laporan',
                '${(data['stats'] as Map?)?['total_reports'] ?? 0}'),
            _DetailRow('Diselesaikan',
                '${(data['stats'] as Map?)?['total_resolved'] ?? 0}'),
              ...
              children: [
                Expanded(
                  child: OutlinedButton.icon(
                    ...
                    icon: const Icon(Icons.star_rounded,
                        size: 16, color: AppColors.bounty),
                    label: const Text('Edit Poin',
                        style: TextStyle(color: AppColors.bounty)),
                    style: OutlinedButton.styleFrom(
                        side: const BorderSide(color: AppColors.bounty)),
                  ),
                ),
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () {
                      Navigator.pop(ctx);
                      _editRole(context, userId, data['role'] ?? 'user');
                    },
                      ...
                      child: ListTile(
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            const Icon(Icons.star_rounded,
                                size: 12, color: AppColors.bounty),
                            Text(' ${data['total_poin'] ?? 0}',
                                style: const TextStyle(
                                    fontSize: 11,
                        ...
                        trailing: IconButton(
                          icon: const Icon(Icons.more_vert_rounded,
                              color: AppColors.textSecondary),
                          onPressed: () =>
                              _showUserDetail(context, data, userId),
                        ),
